...
class IngresoFormTests(TestCase):
    ...
    def test_fecha_ingreso_debe_ser_hoy(self):
        hoy_cr = timezone.now().astimezone(ZoneInfo("America/Costa_Rica")).date()
        ayer = (hoy_cr - timedelta(days=1)).isoformat()
    ...
    def test_fecha_ingreso_hoy_es_valida(self):
        hoy = timezone.now().astimezone(ZoneInfo("America/Costa_Rica")).date().isoformat()
